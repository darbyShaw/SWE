...
class GPI_SWE
{
  ...
  int argc;
  char** argv;
  gaspi_group_t l_group;
};

GPI_SWE::GPI_SWE (int l_argc, char** l_argv) : 
            l_t (0.0), l_iterations (0), l_startc (1),
            l_boundarySize ({{1, 1, 1, 1}}), argc (l_argc), 
            argv (l_argv), l_group(GASPI_GROUP_ALL) {
  ASSERT(gaspi_config_get(&config));
  config.queue_size_max = 4096;
  ...
  ASSERT(gaspi_barrier(l_group, GASPI_BLOCK));
...
void GPI_SWE::collectStepbyStepStats (int spawned_procs)
{
  ...
  gaspi_allreduce(&group_construct, &total_group_construct, 1,
                  GASPI_OP_SUM, GASPI_TYPE_FLOAT, l_group, GASPI_BLOCK);
  if (l_gpiRank == 0)
  {
  ...
  for(int c=l_startc; c<=l_numberOfCheckPoints; c++) {  
    ...
    while( l_t < l_checkPoints[c] ) {
      ...
      gaspi_allreduce(&l_maxTimeStepWidth, &l_maxTimeStepWidthGlobal, 1,
                      GASPI_OP_MIN, GASPI_TYPE_FLOAT, l_group, GASPI_BLOCK);
      ...
      gaspi_allreduce(&l_iterTime, &l_sumIterTime, 1,
                      GASPI_OP_SUM, GASPI_TYPE_DOUBLE, l_group, GASPI_BLOCK);
      if (l_gpiRank == 0)
      {
    ...
    {
      /*First transfer data to processes not being released.*/
      ASSERT (transferData());
      /*Determine the processes the are leaving us.*/
      gaspi_proc_bits_t proc_bits;
      GASPI_PROC_ARRAY_CREATE(proc_bits, l_numberOfProcesses);
      for (int m = l_numberOfProcesses-1; m >= (l_numberOfProcesses/2); m--)
        GASPI_PROC_BIT_SET(proc_bits, m);
      ASSERT (gaspi_proc_alloc_release (proc_bits, (l_numberOfProcesses/2),
                                        &l_gpiRank, &l_numberOfProcesses, argv));
      collectStepbyStepStats(l_numberOfProcesses);
      GASPI_PROC_ARRAY_RELEASE(proc_bits, (l_numberOfProcesses*2));
...
gaspi_return_t GPI_SWE::transferData()
{
  /*Detach the old segements.*/
  gaspi_segment_delete(0);
  gaspi_segment_delete(1);
  gaspi_segment_delete(2);
  /*Create our new data blocks.*/
  int rows = l_nXLocal+2;
  int cols = l_nYLocal*2+2;
  ...
  /*Attach the new data blocks to segments.*/
  ...
  /*Attach the old data blocks to new segments for data transfer*/
  ...
  ASSERT(gaspi_barrier(l_group, GASPI_BLOCK));
  /*Now transfer the data to appropriate ranks.*/
  gaspi_rank_t parent_rank = l_gpiRank/2;
  int l_newCols = cols;
  int l_oldCols = l_nYLocal+2;
  int l_oldRows = l_nXLocal;
  /*Compute the offsets. Write to first half or second half of the parent.*/
  gaspi_offset_t local_offsets[l_oldRows], remote_offsets[l_oldRows];
  gaspi_size_t sizes[l_oldRows];
  for(int i = 0; i < l_oldRows; i++)
  {
    if ((l_gpiRank % 2) == 0)
    {
      local_offsets[i] = ((i+1)*(l_oldCols) + 1) * sizeof(float);
      remote_offsets[i] = ((i+1)*(l_newCols) + 1) * sizeof(float);
    }
    else
    {
      local_offsets[i] = ((i+1)*(l_oldCols) + 1) * sizeof(float);
      remote_offsets[i] = ((i+1)*(l_newCols) + l_oldCols - 1) * sizeof(float);
    }
    sizes[i] = (l_nYLocal) * sizeof(float);
  }
  /*Write out the data*/
  gaspi_segment_id_t segment_src[l_oldRows];
  gaspi_segment_id_t segment_dest[l_oldRows];
  gaspi_notification_id_t fid = (l_gpiRank % 2) + 1;
  for(int i = 0; i < 3; i++)
  {
  ...
  if (l_gpiRank < (l_numberOfProcesses/2))
  {
    ...
    for(int j = 0; j < 3; j++)
    {
      ...
      gaspi_notification_t num_count = 2;
      while (num_count > 0)
      {
        ASSERT(gaspi_notify_waitsome(j, 1, 2, &fid, GASPI_BLOCK));
        val = 0;
  ...
  /*Free our previous blocks and assign the new ones.*/
  ASSERT(gaspi_barrier(l_group, GASPI_BLOCK));
  free(l_height);
  free(l_discharge_hu);
  free(l_discharge_hv);
...
gaspi_return_t GPI_SWE::recalculateRedistribute (int checkpoint)
{
  ...
  /*This is the bathymetry set function of SWE_BathymetryDamBreakScenario*/
  auto l_fnc = [](float x, float y) 
    {return ( std::sqrt( (x-500.f)*(x-500.f) + (y-500.f)*(y-500.f) ) < 50.f ) ? -255.f: -260.f;};
  l_waveBlock->setBathymetry(l_fnc);
  ASSERT(gaspi_barrier(l_group, GASPI_BLOCK));
...
void GPI_SWE::exchangeBottomTopGhostLayers ()
{
  ...
  if(l_flow.l_topNeighborRank >= 0)
  {
    for(int j = 0; j < 3; j++)
    {
      /*tools::Logger::logger.cout() << "waiting for notification from top rank " 
                << l_flow.l_topNeighborRank << " on segment " << j << std::endl;*/
      ASSERT(gaspi_notify_waitsome(j, l_flow.l_topNeighborRank, 1, &fid, GASPI_BLOCK));
      val = 0;
      ASSERT (gaspi_notify_reset(j, fid, &val));
      /*tools::Logger::logger.cout() << "Received notification from top rank " 
                << l_flow.l_topNeighborRank << " on segment " << j << std::endl;*/
    }
  }
  if(l_flow.l_bottomNeighborRank >= 0)
  {
    for(int j = 0; j < 3; j++)
    {
      /*tools::Logger::logger.cout() << "waiting for notification from bottom rank " 
                << l_flow.l_bottomNeighborRank << " on segment " << j << std::endl;*/
      ASSERT(gaspi_notify_waitsome(j, l_flow.l_bottomNeighborRank, 1, &fid, GASPI_BLOCK));
      val = 0;
      ASSERT (gaspi_notify_reset(j, fid, &val));
      /*tools::Logger::logger.cout() << "Received notification from bottom rank " 
                << l_flow.l_bottomNeighborRank << " on segment " << j << std::endl;*/
    }
  }
}
